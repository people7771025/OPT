...
同 session 後續兩個 commit：(1) **IronCondor 勝率正式修法**——不只改標籤，把 Z-score 分母從 em（≈0.8σ）改成真正 1σ（`em/0.7979`），預設 74%→63%（誠實值）；(2) **滑桿/數字框範圍全站對齊**——健檢只發現幾個，實際每個元件都有，全 35 組配對已用「縮小數字框對齊滑桿」方向逐一對齊（node 驗證 0 不一致、所有預設值在範圍內）。互動實跑因本機 Chrome profile 被佔用仍未做。
